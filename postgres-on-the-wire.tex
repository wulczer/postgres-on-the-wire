...
\begin{frame}
  ...
  % state machine, startup packet, auth request, query<->readyforquery cycle
...
\begin{frame}
  ...
  % SSL negotiation packet drawing
...
\begin{frame}
  ...
  % state machine with SSL negotation added
...
\begin{frame}
  ...
  % diagram of simple query protocol flow
\end{frame}
...
\begin{frame}
  ...
  \uncover<2->{
    \begin{beamercolorbox}[center,sep=1em]{sqlinjection}
      \only<-3>{
        \texttt{SELECT * FROM users WHERE username = 'x\alert{' or 1=1 --}'}
      }
      \only<4->{
        \texttt{SELECT * FROM users WHERE username = \alert{\$1}}
      }
    \end{beamercolorbox}
  }
  \begin{itemize}
  ...
  \uncover<3->{
    \begin{beamercolorbox}[center,sep=1em]{sqlinjection}
      \only<-3>{
        \texttt{SELECT * FROM users WHERE username = 'x\alert{'; drop table invoices;--}'}
      }
      \only<4->{
        \texttt{SELECT * FROM users WHERE username = \alert{\$1}}
    }
...
\begin{frame}
  ...
  % describe message drawing and its responses
\end{frame}
...
\begin{frame}
  ...
  % diagram of extended query protocol flow
\end{frame}
...
\begin{frame}
  ...
  % diagram of COPY subprotocol flow
\end{frame}
